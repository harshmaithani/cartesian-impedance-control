...
float B_x=1;      float B_y=100;   float B_z=100;
float K_x=1;      float K_y=100;   float K_z=500;
...
float v_x=0;	float v_x_0=0; 		float v_x_d = 0;
float v_y=0;	float v_y_0=0;      float v_y_d = 0;
float v_z=0;	float v_z_0=0;      float v_z_d = 0;
...
void commandedposeCallback(const std_msgs::Float64MultiArray::ConstPtr &msg)
{	
        cmd_pose_.data[0]          =      msg->data[0];	
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber commanded_pose_sub = n.subscribe("/commanded_pose",1000,commandedposeCallback); 	                    // Commanded Pose
  ...
  */
   ...
   */
   ...
   //************** Damper Only B(v-vd)=F ***********************
   /*
   v_z_theoretical = (F_z / B_z) + v_z_d;
   // z = v_z*dt;
   // z = v_z*2.5;
...
//position_theoretical_.data[2] = z_theoretical; 
  ...
  position_cmd_.data[2] = current_pose_rpy.position.z + z_cmd;
   ...
   commanded_pose.position.y=current_pose_rpy.position.y;
   commanded_pose.position.z=current_pose_rpy.position.z + z_cmd;  //Active Following
// commanded_pose.position.z=cmd_pose_.data[0] + z_cmd;  // Using input from MATLAB 
// commanded_pose.position.z=0.720 + z_cmd;  //Spring
