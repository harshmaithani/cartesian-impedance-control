...
float M_x=1;   float M_y=1;   float M_z=0.1;   
float B_x=1;   float B_y=1;   float B_z=0.1;
float K_x=1;   float K_y=1;   float K_z=0;
...
int main(int argc, char **argv)
{
   ...
   */
   ...
   a_x=(F_x-B_x*v_x-K_x*x)/M_x;
   v_x=v_x+a_x*dt;
   x=x+v_x*dt;
   
   a_y=(F_y-B_y*v_y-K_y*y)/M_y;
   v_y=v_y+a_y*dt;
   y=y+v_y*dt;
   
   a_z=(F_z-B_z*v_z-K_z*z)/M_z;
 /*
   if (a_z > acc_upper_limit)
   {
   a_z=acc_upper_limit;
   }

   if (a_z < acc_lower_limit)
   {
   a_z=acc_lower_limit;
   }	
*/
...
/* 
   if (v_z > vel_upper_limit)
   {
   v_z=vel_upper_limit;
   }

   if (v_z < vel_lower_limit)
   {
   v_z=vel_lower_limit;
   }
*/
   ...
   acc_.data[0]=a_z;
   vel_.data[0]=v_z;
   displacement_.data[0]=z;

   commanded_pose.position.x=0.078;
   commanded_pose.position.y=0.007;
   commanded_pose.position.z=current_pose_rpy.position.z + z;
