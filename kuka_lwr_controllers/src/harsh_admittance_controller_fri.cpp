#include <harsh_admittance_controller_fri.h>
...
{
    HarshAdmittanceControllerFRI::HarshAdmittanceControllerFRI() {}
    HarshAdmittanceControllerFRI::~HarshAdmittanceControllerFRI() 
    {
	delete RML_;
	delete IP_;
    ...
    bool HarshAdmittanceControllerFRI::init(hardware_interface::PositionJointInterface *robot, ros::NodeHandle &n)
    {
      // ROS_INFO("***** START HarshAdmittanceControllerFRI::init ************");
	
        if( !(KinematicChainControllerBase<hardware_interface::PositionJointInterface>::init(robot, n)) )
        {
            ROS_ERROR("Couldn't initilize HarshAdmittanceControllerFRI controller.");
            return false;
        }
        ...
        sub_command_ = nh_.subscribe("command", 1, &HarshAdmittanceControllerFRI::command, this);
    ...
    void HarshAdmittanceControllerFRI::starting(const ros::Time& time)
    {
		ROS_INFO("***** HarshAdmittanceControllerFRI::starting ************");
		cmd_flag_ = 0;  // set this flag to 0 to not run the update method
    }

    void HarshAdmittanceControllerFRI::update(const ros::Time& time, const ros::Duration& period)
    {
	//ROS_INFO("***** HarshAdmittanceControllerFRI::update debut ************");
        ...
        if (cmd_flag_)
        {
		   ...
		   for (size_t i=0; i<joint_handles_.size(); ++i)
	       {
			...
			IP_->MaxVelocity->VecData[i] = (double)1.0;
			IP_->MaxAcceleration->VecData[i] = (double)1.0;
		    ...
		    {
			ROS_INFO("HarshAdmittanceControllerFRI::update : ERROR during trajectory generation err n°%d",resultValue_);
		    }


            if (Equal(x_, x_des_, 0.0001))
            {
                ROS_INFO("On target");
                cmd_flag_ = 0;
            }
			...
			//ROS_INFO("***** HarshAdmittanceControllerFRI::update fin ************");
		}
    }

    void HarshAdmittanceControllerFRI::command(const kuka_lwr_controllers::PoseRPY::ConstPtr &msg)
    {
		//ROS_INFO("***** START HarshAdmittanceControllerFRI::command ************");
       ...
       // ROS_INFO("***** FINISH HarshAdmittanceControllerFRI::command ************");
    }

}

PLUGINLIB_EXPORT_CLASS(kuka_lwr_controllers::HarshAdmittanceControllerFRI, controller_interface::ControllerBase)
