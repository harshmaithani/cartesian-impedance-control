#include <harsh_admittance_controller.h>
...
{
    HarshAdmittanceController::HarshAdmittanceController() {}
    HarshAdmittanceController::~HarshAdmittanceController() {}

    bool HarshAdmittanceController::init(hardware_interface::PositionJointInterface *robot, ros::NodeHandle &n)
    {
      // ROS_INFO("***** START HarshAdmittanceController::init ************");
	
        if( !(KinematicChainControllerBase<hardware_interface::PositionJointInterface>::init(robot, n)) )
        {
            ROS_ERROR("Couldn't initilize HarshAdmittanceController controller.");
            return false;
        }
        ...
        sub_command_ = nh_.subscribe("command", 1, &HarshAdmittanceController::command, this);
    ...
    void HarshAdmittanceController::starting(const ros::Time& time)
    {
		ROS_INFO("***** HarshAdmittanceController::starting ************");
		cmd_flag_ = 0;  // set this flag to 0 to not run the update method
    }

    void HarshAdmittanceController::update(const ros::Time& time, const ros::Duration& period)
    {
	//ROS_INFO("***** HarshAdmittanceController::update debut ************");
	...
	//ROS_INFO("***** HarshAdmittanceController::update fin ************");
    }

    void HarshAdmittanceController::command(const kuka_lwr_controllers::PoseRPY::ConstPtr &msg)
    {
		//ROS_INFO("***** START HarshAdmittanceController::command ************");
       ...
       // ROS_INFO("***** FINISH HarshAdmittanceController::command ************");
    }

}

PLUGINLIB_EXPORT_CLASS(kuka_lwr_controllers::HarshAdmittanceController, controller_interface::ControllerBase)
