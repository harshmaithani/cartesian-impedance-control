...
namespace kuka_lwr_controllers 
{
    ...
    HarshTrustImpedanceController::~HarshTrustImpedanceController() 
    {
     ...
     bool HarshTrustImpedanceController::init(hardware_interface::KUKACartesianInterface *robot, ros::NodeHandle &n)
     {
	    ...
	    pub_cart_imp_kdl_joint_motor_forces_  	= n.advertise<std_msgs::Float64MultiArray>("/cart_imp_kdl_joint_motor_forces_", 1000); 
       ...
       // Resize variables here
        ...
        robot_joint_motor_forces_.data.resize(6);
        ...
        robot_forces_.resize(6);
        joint_motor_torques_eigen_.resize(kdl_chain_.getNrOfJoints());
        jacobian_eigen_= MatrixXd:: Zero(6,kdl_chain_.getNrOfJoints());
        jacobian_transpose_eigen_= MatrixXd:: Zero(kdl_chain_.getNrOfJoints(),6);
        jacobian_transpose_inverse_eigen_= MatrixXd:: Zero(6,kdl_chain_.getNrOfJoints());
	 ...
	 void HarshTrustImpedanceController::starting(const ros::Time& time)
     {
		...
		KDL::Stiffness init_damping( 0.7, 0.7, 0.7, 0.7, 0.7, 0.7 );
	 ...
	 void HarshTrustImpedanceController::update(const ros::Time& time, const ros::Duration& period)
     {
		...
		*/
		...
		joint_motor_torques_eigen_(0)= joint_handles_[0].getEffort();
		joint_motor_torques_eigen_(1)= joint_handles_[1].getEffort();
		joint_motor_torques_eigen_(2)= joint_handles_[2].getEffort();
		joint_motor_torques_eigen_(3)= joint_handles_[3].getEffort();
		joint_motor_torques_eigen_(4)= joint_handles_[4].getEffort();
		joint_motor_torques_eigen_(5)= joint_handles_[5].getEffort();
		joint_motor_torques_eigen_(6)= joint_handles_[6].getEffort();
		...
		// Robot Motor Forces 
		jacobian_eigen_(0,0)=jacobian_(0,0);
		jacobian_eigen_(0,1)=jacobian_(0,1);
		jacobian_eigen_(0,2)=jacobian_(0,2);
		jacobian_eigen_(0,3)=jacobian_(0,3);
		jacobian_eigen_(0,4)=jacobian_(0,4);
		jacobian_eigen_(0,5)=jacobian_(0,5);
		jacobian_eigen_(0,6)=jacobian_(0,6);
		jacobian_eigen_(1,0)=jacobian_(1,0);
		jacobian_eigen_(1,1)=jacobian_(1,1);
		jacobian_eigen_(1,2)=jacobian_(1,2);
		jacobian_eigen_(1,3)=jacobian_(1,3);
		jacobian_eigen_(1,4)=jacobian_(1,4);
		jacobian_eigen_(1,5)=jacobian_(1,5);
		jacobian_eigen_(1,6)=jacobian_(1,6);
		jacobian_eigen_(2,0)=jacobian_(2,0);
		jacobian_eigen_(2,1)=jacobian_(2,1);
		jacobian_eigen_(2,2)=jacobian_(2,2);
		jacobian_eigen_(2,3)=jacobian_(2,3);
		jacobian_eigen_(2,4)=jacobian_(2,4);
		jacobian_eigen_(2,5)=jacobian_(2,5);
		jacobian_eigen_(2,6)=jacobian_(2,6);
		jacobian_eigen_(3,0)=jacobian_(3,0);
		jacobian_eigen_(3,1)=jacobian_(3,1);
		jacobian_eigen_(3,2)=jacobian_(3,2);
		jacobian_eigen_(3,3)=jacobian_(3,3);
		jacobian_eigen_(3,4)=jacobian_(3,4);
		jacobian_eigen_(3,5)=jacobian_(3,5);
		jacobian_eigen_(3,6)=jacobian_(3,6);
		jacobian_eigen_(4,0)=jacobian_(4,0);
		jacobian_eigen_(4,1)=jacobian_(4,1);
		jacobian_eigen_(4,2)=jacobian_(4,2);
		jacobian_eigen_(4,3)=jacobian_(4,3);
		jacobian_eigen_(4,4)=jacobian_(4,4);
		jacobian_eigen_(4,5)=jacobian_(4,5);
		jacobian_eigen_(4,6)=jacobian_(4,6);
		jacobian_eigen_(5,0)=jacobian_(5,0);
		jacobian_eigen_(5,1)=jacobian_(5,1);
		jacobian_eigen_(5,2)=jacobian_(5,2);
		jacobian_eigen_(5,3)=jacobian_(5,3);
		jacobian_eigen_(5,4)=jacobian_(5,4);
		jacobian_eigen_(5,5)=jacobian_(5,5);
		jacobian_eigen_(5,6)=jacobian_(5,6);
		
		jacobian_transpose_eigen_=jacobian_eigen_.transpose();
		
		pseudo_inverse(jacobian_transpose_eigen_,jacobian_transpose_inverse_eigen_);
		
		robot_forces_ = jacobian_transpose_inverse_eigen_*joint_motor_torques_eigen_;
		
		robot_joint_motor_forces_.data[0] = robot_forces_(0);
		robot_joint_motor_forces_.data[1] = robot_forces_(1);
		robot_joint_motor_forces_.data[2] = robot_forces_(2);
		robot_joint_motor_forces_.data[3] = robot_forces_(3);
		robot_joint_motor_forces_.data[4] = robot_forces_(4);
		robot_joint_motor_forces_.data[5] = robot_forces_(5);
	    ...
	    pub_cart_imp_kdl_joint_motor_forces_.publish(robot_joint_motor_forces_);
